...
This script will:
- Install Prefect 3.1.9 (outside OneDrive in WSL home directory)
- Copy PostgreSQL password to local file (`~/.prefect-postgres-password`)
...
- **User**: `bpo_user`
- **Password**: From `~/.prefect-postgres-password` (local copy) or `ops/secrets/postgres_password.txt`
...
Prefect data is stored **outside OneDrive** to avoid sync conflicts:
- **Location**: `~/prefect-data/` (in WSL2 home directory)
- **Password**: `~/.prefect-postgres-password` (local copy)
- **Backup**: Consider backing up these directories periodically

## Migration Notes

- The Prefect Server Docker container has been removed from `docker-compose.yml`
- Existing Prefect data in `../data/prefect/` can be migrated to `~/prefect-data/` if needed
- The `scraper-core` container now connects to WSL2-hosted Prefect Server
- All Prefect data is stored in WSL filesystem (outside OneDrive) to prevent sync issues
