...
class DomainCrawler(BaseScraper):
    ...
    async def crawl(self, start_url: str) -> Dict[str, Any]:
        ...
        while self.to_visit and self.crawled_count < self.max_pages:
            ...
            # Extract full text content (no boilerplate filtering during scrape)
            # Boilerplate removal will happen only during markdown conversion
            try:
                soup = BeautifulSoup(html_content, 'html.parser')
                main_content = soup.get_text(separator=' ', strip=True)
            except Exception:
                # Fallback: use raw HTML as text
                main_content = html_content
            
            # Save to database (markdown conversion with boilerplate removal happens in _save_page)
    ...
    async def _save_page(
        ...
        """Save page to database. Markdown conversion applies boilerplate removal."""
        try:
            domain = self._get_domain(url)
            content_hash = self.calculate_content_hash(html_content)
            
            # Extract title and stripped text
            soup = None
            try:
                soup = BeautifulSoup(html_content, 'html.parser')
            ...
            # Extract stripped text (visible text only, HTML tags removed)
            # BeautifulSoup4's get_text() is the best method for this
            stripped_text = None
            if soup:
                try:
                    ...
                    # Extract visible text with proper spacing
                    ...
                    logger.debug(f"Error stripping HTML for {url}: {strip_error}")
                    # Fallback: use main_content if stripping fails
                    stripped_text = main_content.strip() if main_content else None
            else:
                # If soup creation failed, use main_content as fallback
                stripped_text = main_content.strip() if main_content else None
            
            # Convert to markdown WITH boilerplate removal (only place where we filter)
            markdown_content = None
            try:
                # Remove boilerplate from HTML before converting to markdown
                cleaned_html = self.boilerplate_detector.remove_boilerplate(html_content)
                ...
                logger.debug(f"Error converting to markdown for {url}: {md_error}")
                # Fallback: use main content as markdown
            ...
            query = """
                INSERT INTO scraped_sites (
                    ...
                    error_message, proxy_used, cost, metadata, markdown_content, stripped_text
                ) VALUES (
                ...
                ON CONFLICT (url) DO UPDATE SET
                    ...
                    stripped_text = EXCLUDED.stripped_text,
